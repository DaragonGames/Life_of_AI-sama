...
public class LocalGPT : MonoBehaviour
{
    ...
    int RunSingleTokenInference(int token)
    {
        var inputIds = new Tensor<int>(new TensorShape(1, 1), new int[] { token });
    ...
    void RunInference()
    {
        int nextToken = RunSingleTokenInference(outputTokens[currentToken]);
